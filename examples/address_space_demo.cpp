...
template <> struct microfmt::formatter<NativeHostTask> {
  ...
  void format(const NativeHostTask &task, const sink &out) const noexcept {
    microfmt::format_to(out, "NativeHostTask(id={}, flags={:#x}, name_ptr={})",
                        task.task_id, task.flags,
...
static_assert(sizeof(Compat32GuestTask) == 16,
              "Compat32GuestTask must be 16 bytes");
static_assert(alignof(Compat32GuestTask) == 4,
              "Compat32GuestTask must be 4-byte aligned");
...
template <> struct microfmt::formatter<Compat32GuestTask> {
  ...
  void format(const Compat32GuestTask &task, const sink &out) const noexcept {
    microfmt::format_to(
        out,
        "Compat32GuestTask(id={}, prio={}, flags={:#x}, comm={}, stack={})",
        task.task_id, task.priority, task.state_flags, task.comm_name,
        task.stack_base);
  }
};
...
int main() {
  microfmt::println("=========================================================="
                    "======================");
  microfmt::println(
      "         microfmt Type-Erased Address Space & Compat32 Demo");
  microfmt::println("=========================================================="
                    "======================");
  ...
  alignas(alignof(NativeHostTask))
      std::byte host_task_scratch[sizeof(NativeHostTask)];
  char host_str_scratch[32];

  microfmt::remote_ref<NativeHostTask> host_task_ref(
      reinterpret_cast<uintptr_t>(&local_task), host_space, host_task_scratch);

  microfmt::remote_string_view host_name_ref(
      reinterpret_cast<uintptr_t>(local_task_name), host_space,
      host_str_scratch);
  ...
  simulated_guest_context guest_ctx{
      .vm_id = 42,
      .host_base_addr = reinterpret_cast<uintptr_t>(simulated_ram),
      .memory_limit = sizeof(simulated_ram)};
  microfmt::address_space_ref guest_space(simulated_guest_space_tag{},
                                          guest_ctx);

  alignas(alignof(Compat32GuestTask))
      std::byte guest_task_scratch[sizeof(Compat32GuestTask)];
  char guest_str_scratch[16]; // Small 16-byte chunk buffer

  microfmt::remote_ref<Compat32GuestTask> guest_task_ref(0x100, guest_space,
                                                         guest_task_scratch);
  ...
  microfmt::remote_ref<Compat32GuestTask> null_task_ref(0, guest_space,
                                                        guest_task_scratch);
  ...
  microfmt::remote_ref<Compat32GuestTask> fault_task_ref(0x2000, guest_space,
                                                         guest_task_scratch);
  microfmt::remote_string_view fault_str_ref(0x2000, guest_space,
                                             guest_str_scratch);
