...
template <> struct microfmt::formatter<FileOperations> {
  ...
  void format(const FileOperations &fops, const sink &out) const noexcept {
    microfmt::format_to(out, "fops(read={:#x}, write={:#x}, release={:#x})", fops.read_fn, fops.write_fn,
                        fops.release_fn);
  }
};
...
int main() {
  ...
  auto *fops = static_cast<FileOperations *>(static_cast<void *>(&simulated_kernel_ram[0x200]));
  fops->read_fn = 0;                  // null
  fops->write_fn = kVBase + 0x1000;   // points to kernel::fs::ext4_write
  fops->release_fn = kVBase + 0x1024; // inside ext4_write+0x24

  kernel_memory_ctx mem_ctx{.ram_base = reinterpret_cast<uintptr_t>(simulated_kernel_ram),
                            .ram_size = sizeof(simulated_kernel_ram)};

  microfmt::address_space_ref space(kernel_space_tag{}, mem_ctx);
  microfmt::symbol_resolver_ref resolver = microfmt::symbol_resolver_ref::make<kernel_sym_tag>();

  char scratch[64];
  microfmt::symbol_resolution_context symbol_context{scratch};
  alignas(alignof(FileOperations)) std::byte fops_scratch[sizeof(FileOperations)];
  ...
  auto valid_ref =
      microfmt::remote_diag_ref<FileOperations>(kVBase + 0x200, space, resolver, fops_scratch, symbol_context);
  microfmt::println("Valid Struct      : {}", valid_ref);

  // Dereference and symbolize function pointers
  auto loaded_fops = valid_ref.load();
  if (loaded_fops) {
    microfmt::remote_fn_ptr fn_write((*loaded_fops)->write_fn, resolver, symbol_context);
    microfmt::remote_fn_ptr fn_rel((*loaded_fops)->release_fn, resolver, symbol_context);
  ...
  auto mod_fault_ref = microfmt::remote_diag_ref<FileOperations>(0xffff'8000'0010'4200ULL, space, resolver,
                                                                 fops_scratch, symbol_context);
  microfmt::println("Module Fault      : {}", mod_fault_ref);

  // Completely unmapped read -> Raw hex fault diagnostic
  auto bad_fault_ref = microfmt::remote_diag_ref<FileOperations>(0x0000'0000'dead'beefULL, space, resolver,
                                                                 fops_scratch, symbol_context);
