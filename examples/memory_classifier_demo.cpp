...
template <>
struct microfmt::memory_classifier_traits<demo_classifier_tag> {
  ...
  static bool classify_address(const void *opaque_context,
    ...
    if (!opaque_context)
      return false;
    const auto &context =
        *static_cast<const demo_classifier_context *>(opaque_context);
...
int main() {
  constexpr microfmt::memory_region_info regions[]{
      ...
      {.start_address = 0x4000'0000,
       ...
       .writable = true,
       .executable = false}};
  const demo_classifier_context context{
      regions, sizeof(regions) / sizeof(regions[0])};
  const auto classifier =
      microfmt::memory_classifier_ref::make<demo_classifier_tag>(context);

  constexpr uintptr_t addresses[]{0x8000'1234, 0x7fff'f000, 0x4000'0040,
                                  0x1234'5678};
  for (uintptr_t address : addresses) {
    ...
    microfmt::println(
        "{:#010x}: {} [{:#010x}, {:#010x}) space={} {}{}{}", address,
        region_name(info.type), info.start_address, info.end_address,
        info.space_id, info.readable ? "r" : "-",
