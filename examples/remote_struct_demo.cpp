...
int main() {
  ...
  SimulatedMemorySpace memory_sim{&remote_instance, 0x50001000, "database_daemon"};

  // Create a type-erased address space reference bound to our simulated memory
  auto space = microfmt::address_space_ref::make<SimulatedMemorySpace::tag>(memory_sim);
  ...
  microfmt::remote_object_view remote_obj(0x10002000, // Remote address of the struct
