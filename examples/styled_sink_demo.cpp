...
int main() {
  ...
  std::printf("transformed: %.*s", static_cast<int>(transformed_output.size()), transformed_output.view().data());
  ...
  std::printf("prefixed:\n%.*s", static_cast<int>(prefixed_output.size()), prefixed_output.view().data());

  microfmt::buffer_sink<256> limited_output;
  microfmt::limit_sink packet_preview(limited_output.as_sink(), 24);
  microfmt::format_to(packet_preview.as_sink(), "packet={} payload={}", 17, "0123456789abcdef");
