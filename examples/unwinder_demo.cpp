...
bool write_arch_register(void *ctx, microfmt::address_space_ref, uint32_t dwarf_reg, const void *in_value,
                         size_t value_size) noexcept {
...
int main() {
  ...
  // User Stack Frames
  auto *user_f1 = static_cast<uintptr_t *>(static_cast<void *>(&simulated_ram[0x600])); // main frame
  user_f1[0] = 0;                                                                       // Terminating frame
  user_f1[1] = 0;

  auto *user_f0 = static_cast<uintptr_t *>(static_cast<void *>(&simulated_ram[0x500])); // query() frame
  ...
  *static_cast<uint64_t *>(static_cast<void *>(&pt_regs[0x70])) = 0x0E; // Vector 14: Page Fault
  *static_cast<uint64_t *>(static_cast<void *>(&pt_regs[0x78])) =
      0x0000'0000'0040'1044ULL;                                         // Interrupted PC: query+0x44
  *static_cast<uint64_t *>(static_cast<void *>(&pt_regs[0x80])) = 0x33; // User CS
  *static_cast<uint64_t *>(static_cast<void *>(&pt_regs[0x90])) = reinterpret_cast<uintptr_t>(user_f0); // User RSP
  *static_cast<uint64_t *>(static_cast<void *>(&pt_regs[0x20])) = reinterpret_cast<uintptr_t>(user_f0); // User RBP

  // Kernel Stack Frames
  auto *k_f1 = static_cast<uintptr_t *>(static_cast<void *>(&simulated_ram[0x100])); // asm_exc_page_fault
  k_f1[0] = 0;                                                                       // End of kernel FP chain
  k_f1[1] = 0xffff'8000'0010'0210ULL;                                                // PC inside asm_exc_page_fault

  auto *k_f0 = static_cast<uintptr_t *>(static_cast<void *>(&simulated_ram[0x080])); // page_fault_handler
  ...
  microfmt::symbol_resolver_ref resolver = microfmt::symbol_resolver_ref::make<symbol_tag>();
  ...
  uintptr_t initial_pc = 0xffff'8000'0010'0020ULL; // PC inside page_fault_handler
  ...
  microfmt::hybrid_backtrace_view bt(unwinder, resolver, symbol_context);
