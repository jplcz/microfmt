...
bool read_arm_register(const void *ctx, microfmt::address_space_ref, uint32_t dwarf_reg, void *out_value,
...
bool write_arm_register(void *ctx, microfmt::address_space_ref, uint32_t dwarf_reg, const void *in_value,
                        size_t value_size) noexcept {
...
template <> struct microfmt::frame_unwinder_traits<standard_fp_unwinder_tag> {
  ...
  static bool step(const void *ctx, microfmt::register_context_ref reg_ctx, uintptr_t &next_fp,
    ...
    if (!ctx || !reg_ctx.read(microfmt::dwarf::arm32::FP, current_fp) || current_fp == 0 || (current_fp % 4) != 0)
      return false;
    ...
    return reg_ctx.write(microfmt::dwarf::arm32::FP, saved_fp) && reg_ctx.write(microfmt::dwarf::arm32::LR, return_lr);
  }
};
...
int main() {
  ...
  auto *exidx_region = static_cast<uint32_t *>(static_cast<void *>(&target_memory[exidx_table_base]));
  ...
  hint_context.add_hint({.pc_start = 0x0800'8000,
                         .pc_end = 0x0800'8200,
                         .routine = [](microfmt::address_space_ref space, uintptr_t current_fp, uintptr_t,
  ...
  microfmt::unwind_hint_registry_ref hint_ref(microfmt::unwind_hint_registry_tag{}, hint_context);

  // D. Setup Simulated Stack Frames at Offset 0x500
  uintptr_t stack_base = 0x500;
  auto *stack_region = static_cast<uint32_t *>(static_cast<void *>(&target_memory[stack_base]));
  ...
  microfmt::symbol_resolver_ref resolver = microfmt::symbol_resolver_ref::make<full_demo_sym_tag>();
  ...
  microfmt::frame_unwinder_ref exidx_unwinder(microfmt::arm_exidx_unwinder_tag{}, exidx_ctx);
  ...
  microfmt::chained_unwinder_context<microfmt::arm_abi_traits> chained_ctx{
      .space = space, .exidx_unwinder = exidx_unwinder, .fp_unwinder = fp_unwinder, .hints = hint_ref};
  microfmt::frame_unwinder_ref robust_unwinder(microfmt::chained_unwinder_tag<microfmt::arm_abi_traits>{}, chained_ctx);
  ...
  microfmt::register_context_ref register_context(&register_file, {&read_arm_register, &write_arm_register}, space,
                                                  register_scratch);
  microfmt::frame_pointer_iterator it(robust_unwinder, register_context, initial_fp, initial_pc);
  ...
  register_file.values[microfmt::dwarf::arm32::SP] = initial_fp;
  register_file.values[microfmt::dwarf::arm32::LR] = initial_pc;
  microfmt::frame_pointer_iterator verbose_it(robust_unwinder, register_context, initial_fp, initial_pc);
