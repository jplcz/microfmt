...
template <> struct formatter<GeoLocation> {
  ...
  void format(const GeoLocation &loc, const sink &out) const noexcept {
    if (!float_spec.empty()) {
      microfmt::format_to(out, "Geo({:{}f}, {:{}f})", loc.latitude, float_spec,
                          loc.longitude, float_spec);
    } else {
...
void demo_stream_tokens_with_runtime_join() {
  ...
  microfmt::println("Subrange: [{}]", microfmt::join(stream.begin() + 1,
                                                     stream.begin() + 4, ", "));
  microfmt::println();
}
...
void demo_compile_time_nttp_join_as() {
  ...
  microfmt::println("NTTP Hex Spec: [{}]",
                    microfmt::join_as<":", "04X">(tokens));
  microfmt::println();
}
#endif
