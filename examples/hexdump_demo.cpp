...
int main() {
  ...
  const uint8_t packet[] = {0x45, 0x00, 0x00, 0x3c, 0x1c, 0x46, 0x40, 0x00, 0x40, 0x06, 0xb1, 0xe6, 0xc0, 0xa8,
                            0x01, 0x64, 0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x20, 0x57, 0x6f, 0x72, 0x6c, 0x64, 0x21};
  ...
  microfmt::format_to(uart_sink, "Packet Dump ({} bytes):\n{}", sizeof(packet),
  ...
  auto buf = microfmt::format<512>("{}", microfmt::hexdump(microfmt::span<const uint8_t>(packet, sizeof(packet)),
  ...
  auto struct_dump = microfmt::format<512>(
      ...
      microfmt::hexdump(microfmt::span<const uint8_t>(reinterpret_cast<const uint8_t *>(&config), sizeof(config)),
                        reinterpret_cast<uintptr_t>(&config)));

  std::printf("%.*s\n", static_cast<int>(struct_dump.size()), struct_dump.view().data());
