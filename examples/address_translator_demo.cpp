...
template <>
struct microfmt::address_translator_traits<demo_translator_tag> {
  ...
  static bool translate(const void *opaque_context, uintptr_t virtual_address,
    ...
    if (!opaque_context)
      return false;
    const auto &context =
        *static_cast<const demo_translator_context *>(opaque_context);
...
int main() {
  constexpr demo_mapping mappings[]{
      {.virtual_base = 0x0000'4000,
  ...
  const demo_translator_context context{mappings,
                                        sizeof(mappings) / sizeof(mappings[0])};
  const auto translator =
      microfmt::address_translator_ref::make<demo_translator_tag>(context);
  ...
  for (uintptr_t virtual_address : addresses) {
    ...
    microfmt::println(
        "VA {:#010x} -> PA {:#010x} space={} {}{}{}{}",
        virtual_address, attributes.physical_address, attributes.space_id,
        attributes.readable ? "r" : "-",
        attributes.writable ? "w" : "-",
        attributes.executable ? "x" : "-",
        attributes.user_accessible ? " user" : " kernel");
  }
}
