...
int main() {
  ...
  microfmt::format_to(output, "safe temperatures: {}\n",
                      microfmt::filter(temperatures, [](int16_t value) noexcept { return value >= 0 && value <= 85; }));
  microfmt::format_to(output, "high registers: {:c04X}\n",
                      microfmt::filter(registers, [](uint16_t value) noexcept { return value >= 0x0010; }));
  microfmt::format_to(output, "odd register values: {:n02x}\n",
                      microfmt::filter(registers, size_t{4}, [](uint16_t value) noexcept { return value % 2 != 0; }));
}
