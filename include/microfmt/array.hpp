...
template <typename T, std::size_t N>
struct tuple_size<microfmt::array<T, N>> : std::integral_constant<std::size_t, N> {};

template <std::size_t I, typename T, std::size_t N> struct tuple_element<I, microfmt::array<T, N>> {
  static_assert(I < N, "array index out of bounds");
  using type = T;
};
