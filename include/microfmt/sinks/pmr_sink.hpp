...
[[nodiscard]] std::pmr::string format(std::pmr::memory_resource *mr, microfmt::string_view fmt_str,
                                      const Args &...args) {
  std::pmr::string result(mr);

  auto out_sink = sink{&result, [](void *ctx, microfmt::string_view sv) noexcept {
...
template <typename... Args>
[[nodiscard]] std::pmr::vector<char> format_vector(std::pmr::memory_resource *mr, microfmt::string_view fmt_str,
                                                   const Args &...args) {
...
public:
  ...
  arena_sink(arena_sink &&other) noexcept
      : mr_(other.mr_), current_chunk_(other.current_chunk_), size_(other.size_), current_cap_(other.current_cap_) {
...
private:
  ...
  [[nodiscard]] static std::size_t allocation_size(std::size_t capacity) noexcept {
  ...
  void release_chunks() noexcept {
    while (current_chunk_ != nullptr) {
      ...
      mr_->deallocate(current_chunk_, allocation_size(capacity), alignof(chunk));
      current_chunk_ = previous;
    }
  }

  value_ptr<std::pmr::memory_resource> mr_;
