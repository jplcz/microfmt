...
template <size_t Capacity> class ring_buffer_sink {
  ...
  static_assert((Capacity & (Capacity - 1)) == 0, "Capacity must be a power of 2 for fast bitmask wrapping");
...
public:
  ...
  [[nodiscard]] bool full() const noexcept { return count_.load(std::memory_order_relaxed) >= Capacity; }
