 ...
 * poisoned and any further access aborts via @c MICROFMT_ASSERT, on every
 ...
 *   `JPLCZ_MICROFMT_ENABLE_STRICT_WARNINGS`/`JPLCZ_MICROFMT_ENABLE_WERROR`).
...
private:
  static constexpr T take_from(checked_value &other) noexcept {
    MICROFMT_ASSERT(!other.moved_from_, "checked_value: move from an already moved-from value");
 ...
 * pointer both trap via `MICROFMT_ASSERT` before any dereference happens.
...
public:
   ...
   *
   * Explicitly-unsafe tier: only a `MICROFMT_DEBUG_ASSERT`, so it is compiled
   * out under `NDEBUG` (unless `MICROFMT_DEBUG` is also defined). Still does
   ...
   * checks.
   *
   * Explicitly-unsafe tier: only `MICROFMT_DEBUG_ASSERT`s, so both checks
   * compile out under `NDEBUG` (unless `MICROFMT_DEBUG` is also defined).
