...
struct arm_exidx_unwinder_context {
  ...
  /**
   * @brief Caller-supplied off-stack storage for register state
...
template <> struct frame_unwinder_traits<arm_exidx_unwinder_tag> {
  ...
  /**
   * @brief Unwinds one frame using the module's `.ARM.exidx` tables.
   *
   * Reads the saved FP and LR from the stack, binds the return address to an
   * ELF module, binary-searches its EXIDX section, and executes the matching
   * inline or `.ARM.extab` bytecode.
   *
...
struct arm_exidx_unwinder_holder {
  ...
  /// Off-stack register state scratch.
