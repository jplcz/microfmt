 ...
 */
using remote_field_format_fn = void (*)(address_space_ref space,
...
public:
  ...
  explicit string_ptr(const char *ptr) noexcept
  ...
  as_view(address_space_ref space, span<char> scratch,
...
static_assert(alignof(string_ptr) == sizeof(void *),
              "string32_ptr must be naturally aligned");
...
public:
  ...
  [[nodiscard]] constexpr uintptr_t address() const noexcept {
    return static_cast<uintptr_t>(addr_);
  }
  [[nodiscard]] constexpr bool is_null() const noexcept { return addr_ == 0; }
  [[nodiscard]] constexpr explicit operator bool() const noexcept {
    return addr_ != 0;
...
static_assert(alignof(string32_ptr) == 4,
              "string32_ptr must be 4-byte aligned");
...
template <typename T, typename Enable = void> struct remote_object_traits {
  ...
  static constexpr span<const remote_field_desc> fields() noexcept {
    return {};
  }
...
class remote_object_view {
public:
  template <typename T>
  constexpr remote_object_view(uintptr_t addr, address_space_ref space,
                               type_tag<T>, span<std::byte> scratch) noexcept
      : addr_(addr), space_(space), fields_(remote_object_traits<T>::fields()),
        struct_size_(remote_object_traits<T>::struct_size),
        struct_align_(remote_object_traits<T>::struct_align),
        scratch_(scratch) {
    static_assert(remote_object_traits<T>::is_registered,
                  "Type T must be registered using MICROFMT_REMOTE_STRUCT");
  }

  constexpr remote_object_view(uintptr_t addr, address_space_ref space,
                               span<const remote_field_desc> fields,
                               size_t struct_size, size_t struct_align,
                               span<std::byte> scratch) noexcept
      : addr_(addr), space_(space), fields_(fields), struct_size_(struct_size),
        struct_align_(struct_align), scratch_(scratch) {}

  [[nodiscard]] constexpr uintptr_t address() const noexcept { return addr_; }
  [[nodiscard]] constexpr address_space_ref space() const noexcept {
    return space_;
  }
  [[nodiscard]] constexpr span<const remote_field_desc>
  fields() const noexcept {
    return fields_;
  }
  [[nodiscard]] constexpr size_t struct_size() const noexcept {
    return struct_size_;
  }
...
template <> struct formatter<remote_object_view> {
  ...
  void format(const remote_object_view &view, const sink &out) const noexcept {
    ...
    if (scratch.size() > view.struct_size()) {
      work_buf = span<char>(
          reinterpret_cast<char *>(scratch.data() + view.struct_size()),
