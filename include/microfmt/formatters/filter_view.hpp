...
[[nodiscard]] constexpr auto filter(const Range &rng, Predicate &&pred) noexcept {
  return filtered_range_view<Range, std::decay_t<Predicate>>{rng, std::forward<Predicate>(pred)};
}

template <typename Range, typename Predicate, std::enable_if_t<!std::is_lvalue_reference_v<Range>, int> = 0>
[[nodiscard]] constexpr auto filter(Range &&, Predicate &&) noexcept = delete;

/** Create a filtered view over @p count elements beginning at @p ptr. */
template <typename T, typename Predicate>
[[nodiscard]] constexpr auto filter(const T *ptr, size_t count, Predicate &&pred) noexcept {
  return filtered_pointer_range_view<T, std::decay_t<Predicate>>{ptr, ptr == nullptr ? 0 : count,
 ...
 */
template <typename Range, typename Predicate> struct formatter<filtered_range_view<Range, Predicate>> {
  char open_c{'['};
  char close_c{']'};
  ...
  void format(const filtered_range_view<Range, Predicate> &fv, const sink &out) const noexcept {
    if (open_c != '\0') {
      out.put(open_c);
    }
 ...
 */
template <typename T, typename Predicate> struct formatter<filtered_pointer_range_view<T, Predicate>> {
  char open_c{'['};
  char close_c{']'};
  microfmt::string_view forwarded_spec{""};
  ...
  void format(const filtered_pointer_range_view<T, Predicate> &fv, const sink &out) const noexcept {
    if (open_c != '\0') {
      out.put(open_c);
    }
