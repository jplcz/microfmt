...
[[nodiscard]] constexpr i2c_flags operator|(i2c_flags a, i2c_flags b) noexcept {
  return static_cast<i2c_flags>(static_cast<uint8_t>(a) | static_cast<uint8_t>(b));
}
...
[[nodiscard]] constexpr i2c_msg_view i2c_write(uint8_t addr_7bit, span<const uint8_t> data,
                                               i2c_status status = i2c_status::ok) noexcept {
  return {static_cast<uint16_t>(addr_7bit & 0x7F), i2c_dir::write, data, status, i2c_flags::none};
}

template <size_t N>
[[nodiscard]] constexpr i2c_msg_view i2c_write(uint8_t addr_7bit, const uint8_t (&arr)[N],
                                               i2c_status status = i2c_status::ok) noexcept {
  return {static_cast<uint16_t>(addr_7bit & 0x7F), i2c_dir::write, span<const uint8_t>(arr, N), status,
          i2c_flags::none};
}

// Standard 7-bit I2C read transaction
/** @brief Creates a 7-bit I2C read transaction view. */
[[nodiscard]] constexpr i2c_msg_view i2c_read(uint8_t addr_7bit, span<const uint8_t> data,
                                              i2c_status status = i2c_status::ok) noexcept {
  return {static_cast<uint16_t>(addr_7bit & 0x7F), i2c_dir::read, data, status, i2c_flags::none};
}

template <size_t N>
[[nodiscard]] constexpr i2c_msg_view i2c_read(uint8_t addr_7bit, const uint8_t (&arr)[N],
                                              i2c_status status = i2c_status::ok) noexcept {
  return {static_cast<uint16_t>(addr_7bit & 0x7F), i2c_dir::read, span<const uint8_t>(arr, N), status, i2c_flags::none};
}

// 10-bit Extended address message
/** @brief Creates a 10-bit I2C read or write transaction view. */
[[nodiscard]] constexpr i2c_msg_view i2c_10bit(uint16_t addr_10bit, i2c_dir dir, span<const uint8_t> data,
                                               i2c_status status = i2c_status::ok) noexcept {
  return {static_cast<uint16_t>(addr_10bit & 0x3FF), dir, data, status, i2c_flags::ten_bit};
}
...
template <> struct formatter<i2c_msg_view> {
  ...
  [[nodiscard]] static constexpr microfmt::string_view status_name(i2c_status status) noexcept {
    switch (status) {
    case i2c_status::ok:
      return "OK";
