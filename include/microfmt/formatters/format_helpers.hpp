...
template <typename T>
[[nodiscard]] constexpr hex_view<T> hex(T val, int width = 0, bool prefix = false, bool uppercase = false) noexcept {
...
template <typename T>
[[nodiscard]] constexpr bin_view<T> bin(T val, int width = sizeof(T) * 8, bool prefix = false) noexcept {
...
[[nodiscard]] constexpr bytes_view bytes(uint64_t count) noexcept { return bytes_view{count}; }
...
[[nodiscard]] constexpr addr_offset_view addr_offset(uintptr_t addr, uintptr_t base) noexcept {
...
[[nodiscard]] constexpr memory_range_view mem_range(uintptr_t start, uintptr_t end) noexcept {
...
[[nodiscard]] constexpr aligned_text_view align(microfmt::string_view text, size_t width,
                                                align_mode mode = align_mode::left, char pad_char = ' ') noexcept {
...
template <typename T>
[[nodiscard]] constexpr joined_span_view<T> join(span<const T> items, microfmt::string_view delim = ", ") noexcept {
