...
inline constexpr uint8_t BREAK_BYTE = 0xFF; // Stop code for indefinite containers

inline void encode_header(const sink &out, uint8_t major, uint64_t val) noexcept {
  if (val < 24) {
...
public:
  ...
  map_writer(map_writer &&other) noexcept : out_(std::move(other.out_)), closed_(other.closed_) {
    other.closed_ = true;
  }
  ...
  map_writer &kv(microfmt::string_view k, const char *val) noexcept {
    ...
    if (val == nullptr) {
      ...
      detail::encode_header(out_, detail::MT_TEXT, microfmt::string_view(val).size());
      out_.write(val);
    }
    return *this;
  }

  template <std::size_t N> map_writer &kv(microfmt::string_view k, const char (&val)[N]) noexcept {
  ...
  map_writer &kv(microfmt::string_view k, T val) noexcept {
    ...
    if constexpr (std::is_signed_v<T>) {
      if (val < 0) {
        detail::encode_header(out_, detail::MT_NEGATIVE, static_cast<uint64_t>(-1 - val));
      } else {
        detail::encode_header(out_, detail::MT_UNSIGNED, static_cast<uint64_t>(val));
      }
    } else {
      detail::encode_header(out_, detail::MT_UNSIGNED, static_cast<uint64_t>(val));
    }
...
public:
  ...
  array_writer(array_writer &&other) noexcept : out_(std::move(other.out_)), closed_(other.closed_) {
    other.closed_ = true;
  }
  ...
  array_writer &val(T v) noexcept {
    if constexpr (std::is_signed_v<T>) {
      if (v < 0) {
        ...
        detail::encode_header(out_, detail::MT_UNSIGNED, static_cast<uint64_t>(v));
      }
    } else {
      detail::encode_header(out_, detail::MT_UNSIGNED, static_cast<uint64_t>(v));
    }
...
template <typename Fn> struct formatter<cbor::cbor_map_view<Fn>> {
  ...
  void format(const cbor::cbor_map_view<Fn> &v, const sink &out) const noexcept {
