...
[[nodiscard]] constexpr raw_ptr_view raw_ptr(const T *ptr, ptr_width_mode mode = ptr_width_mode::native) noexcept {
 ...
 *  formatted as null. */
[[nodiscard]] constexpr raw_ptr_view raw_ptr(std::nullptr_t, ptr_width_mode mode = ptr_width_mode::native) noexcept {
  return raw_ptr_view{0, true, mode};
}

[[nodiscard]] constexpr raw_ptr_view raw_ptr(uintptr_t addr, ptr_width_mode mode = ptr_width_mode::native) noexcept {
  return raw_ptr_view{addr, addr == 0, mode};
}

/** Create a raw address view with an 8-hex-digit, 32-bit compatible width. */
template <typename T> [[nodiscard]] constexpr raw_ptr_view raw_ptr32(const T *ptr) noexcept {
...
template <typename T> [[nodiscard]] constexpr raw_range_view<T> raw_range(const T *first, const T *last) noexcept {
...
template <> struct formatter<raw_ptr_view> {
  ...
  MICROFMT_API void format(const raw_ptr_view &ptr, const sink &out) const noexcept;
};
