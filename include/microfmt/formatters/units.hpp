...
[[nodiscard]] constexpr auto auto_si(T val, microfmt::string_view unit, uint8_t precision = 2) noexcept {
  return auto_unit_view<T>{val, unit, scale_base::decimal, precision};
}

// Auto-scaling IEC binary units (B / Bytes)
template <typename T, std::enable_if_t<std::is_integral_v<T>, int> = 0>
[[nodiscard]] constexpr auto auto_bytes(T bytes, uint8_t precision = 2) noexcept {
  return auto_unit_view<T>{bytes, "B", scale_base::binary, precision};
}

// Fixed unit tag (no scaling)
template <typename T> [[nodiscard]] constexpr auto with_unit(T val, microfmt::string_view unit) noexcept {
...
template <typename T> struct formatter<auto_unit_view<T>> {
  ...
  void format(const auto_unit_view<T> &auv, const sink &out) const noexcept {
    ...
    uint64_t abs_val = is_negative ? static_cast<uint64_t>(-auv.value) : static_cast<uint64_t>(auv.value);
