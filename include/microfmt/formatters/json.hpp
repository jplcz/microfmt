  ...
  if constexpr (std::is_signed<T>::value) {
    if (value < 0) {
      ...
      const auto magnitude =
          static_cast<unsigned_type>(0) - static_cast<unsigned_type>(value);
      microfmt::detail::format_unsigned(out, static_cast<uint64_t>(magnitude),
                                        10, false, 0);
      return;
    }
  }
  microfmt::detail::format_unsigned(
      out, static_cast<uint64_t>(static_cast<unsigned_type>(value)), 10, false,
      0);
}
...
template <typename Fn> struct formatter<json::json_obj_view<Fn>> {
  ...
  void format(const json::json_obj_view<Fn> &v,
