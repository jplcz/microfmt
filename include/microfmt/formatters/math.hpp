...
template <typename T, size_t N> [[nodiscard]] constexpr auto vec(const T (&arr)[N]) noexcept {
  return vec_view<T, N>{span<const T>(arr, N)};
}

template <typename T> [[nodiscard]] constexpr auto vec3(const T &x, const T &y, const T &z) noexcept {
...
template <typename T, size_t Rows, size_t Cols> [[nodiscard]] constexpr auto mat(const T (&arr)[Rows * Cols]) noexcept {
  return matrix_view<T, Rows, Cols>{span<const T>(arr, Rows * Cols)};
}

template <typename T, size_t Rows, size_t Cols> struct formatter<matrix_view<T, Rows, Cols>> {
  microfmt::string_view forwarded_spec{""};

  constexpr void parse(format_parse_context &ctx) noexcept { forwarded_spec = ctx.spec(); }

  void format(const matrix_view<T, Rows, Cols> &m, const sink &out) const noexcept {
    out.write("[\n");
    formatter<T> elem_fmt;
    format_parse_context elem_ctx(forwarded_spec);
    elem_fmt.parse(elem_ctx);
