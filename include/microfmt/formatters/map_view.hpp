...
template <typename KeyExtractor, typename ValExtractor, typename Iterator, typename Sentinel> struct map_range_view {
  Iterator first;
  Sentinel last;
  KeyExtractor key_fn;
  ValExtractor val_fn;
...
template <typename T> using remove_cvref_map_t = typename std::remove_cv<typename std::remove_reference<T>::type>::type;

template <typename T, typename = void> struct has_first_member : std::false_type {};

template <typename T>
struct has_first_member<T, std::void_t<decltype(std::declval<const T &>().first)>> : std::true_type {};

template <typename T, typename = void> struct has_second_member : std::false_type {};

template <typename T>
struct has_second_member<T, std::void_t<decltype(std::declval<const T &>().second)>> : std::true_type {};
...
template <typename T, typename = void> struct has_value_method : std::false_type {};
...
template <typename T> struct is_dereferenceable<T, std::void_t<decltype(*std::declval<T &>())>> : std::true_type {};
...
template <typename MapContainer> [[nodiscard]] constexpr auto map_view(const MapContainer &m) noexcept {
  return map_range_view<default_key_fn, default_val_fn, decltype(m.begin()), decltype(m.end())>{
      m.begin(), m.end(), default_key_fn{}, default_val_fn{}};
}

template <typename MapContainer, std::enable_if_t<!std::is_lvalue_reference_v<MapContainer>, int> = 0>
...
[[nodiscard]] constexpr auto map_view(const MapContainer &m, KeyFn &&kfn, ValFn &&vfn) noexcept {
  return map_range_view<std::decay_t<KeyFn>, std::decay_t<ValFn>, decltype(m.begin()), decltype(m.end())>{
      m.begin(), m.end(), std::forward<KeyFn>(kfn), std::forward<ValFn>(vfn)};
}

template <typename MapContainer, typename KeyFn, typename ValFn,
          std::enable_if_t<!std::is_lvalue_reference_v<MapContainer>, int> = 0>
[[nodiscard]] constexpr auto map_view(MapContainer &&, KeyFn &&, ValFn &&) noexcept = delete;

/** Create a view over an iterator/sentinel range with optional extractors. */
template <typename Iterator, typename Sentinel, typename KeyFn = default_key_fn, typename ValFn = default_val_fn,
          typename std::enable_if<detail::is_dereferenceable<Iterator>::value, int>::type = 0>
[[nodiscard]] constexpr auto map_view(Iterator first, Sentinel last, KeyFn &&kfn = KeyFn{},
                                      ValFn &&vfn = ValFn{}) noexcept {
  return map_range_view<std::decay_t<KeyFn>, std::decay_t<ValFn>, Iterator, Sentinel>{
      first, last, std::forward<KeyFn>(kfn), std::forward<ValFn>(vfn)};
}
 ...
 *  for example `{:b04X}`.
 */
template <typename KeyExtractor, typename ValExtractor, typename Iterator, typename Sentinel>
struct formatter<map_range_view<KeyExtractor, ValExtractor, Iterator, Sentinel>> {
  char open_c{'{'};
  char close_c{'}'};
  ...
  void format(const map_range_view<KeyExtractor, ValExtractor, Iterator, Sentinel> &view,
