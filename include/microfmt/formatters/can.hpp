...
[[nodiscard]] constexpr can_flags operator|(can_flags a, can_flags b) noexcept {
  return static_cast<can_flags>(static_cast<uint8_t>(a) |
...
can_frame(uint16_t id, span<const uint8_t> payload, bool rtr = false) noexcept {
  return {static_cast<uint32_t>(id & 0x7FF), payload,
          rtr ? can_flags::rtr : can_flags::none};
}

template <size_t N>
[[nodiscard]] constexpr can_frame_view
can_frame(uint16_t id, const uint8_t (&arr)[N], bool rtr = false) noexcept {
  return {static_cast<uint32_t>(id & 0x7FF), span<const uint8_t>(arr, N),
          rtr ? can_flags::rtr : can_flags::none};
}
...
[[nodiscard]] constexpr can_frame_view can_extended(uint32_t id,
  ...
  return {id & 0x1FFFFFFF, payload,
          can_flags::extended | (rtr ? can_flags::rtr : can_flags::none)};
}
...
template <> struct formatter<can_frame_view> {
  bool candump_style{
      false}; // Format like Linux socketcan candump: can0 123#DEADBEEF
