...
[[nodiscard]] constexpr spi_transfer_view
spi_duplex(const uint8_t (&mosi)[N1], const uint8_t (&miso)[N2], uint8_t cs = 0,
           spi_mode mode = spi_mode::mode0,
           spi_status status = spi_status::ok) noexcept {
  return {span<const uint8_t>(mosi, N1), span<const uint8_t>(miso, N2), cs,
          mode, status};
}
...
template <size_t N>
[[nodiscard]] constexpr spi_transfer_view
spi_write(const uint8_t (&mosi)[N], uint8_t cs = 0,
          spi_mode mode = spi_mode::mode0,
          spi_status status = spi_status::ok) noexcept {
  return {span<const uint8_t>(mosi, N), span<const uint8_t>{}, cs, mode,
          status};
}
...
template <size_t N>
[[nodiscard]] constexpr spi_transfer_view
spi_read(const uint8_t (&miso)[N], uint8_t cs = 0,
         spi_mode mode = spi_mode::mode0,
         spi_status status = spi_status::ok) noexcept {
  return {span<const uint8_t>{}, span<const uint8_t>(miso, N), cs, mode,
          status};
}
...
template <> struct formatter<spi_transfer_view> {
  ...
  void format(const spi_transfer_view &tx, const sink &out) const noexcept {
    const char *hex = uppercase_hex ? "0123456789ABCDEF" : "0123456789abcdef";
    ...
    const size_t total_len =
        (tx.mosi.size() >= tx.miso.size()) ? tx.mosi.size() : tx.miso.size();
