 ...
 */
inline constexpr parsed_float_spec parse_float_spec(microfmt::string_view spec) noexcept {
  ...
  if (pos < len) {
    char ch = spec[pos];
    if (ch == 'f' || ch == 'F' || ch == 'e' || ch == 'E' || ch == 'g' || ch == 'G' || ch == 'a' || ch == 'A') {
      res.type = ch;
    }
  }
...
template <> struct formatter<float> {
   ...
   */
  constexpr void parse(format_parse_context &ctx) noexcept { spec_ = detail::parse_float_spec(ctx.spec()); }
