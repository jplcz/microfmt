...
public:
  ...
  constexpr span(T *first, T *last) noexcept : m_ptr(first), m_size(0) {
    MICROFMT_ASSERT(first != nullptr || last == nullptr, "non-empty span requires non-null data");
    MICROFMT_ASSERT(last != nullptr || first == nullptr, "span end must not be null");
    if (first != nullptr && last != nullptr) {
      MICROFMT_ASSERT(last >= first, "span end must not precede span begin");
  ...
  subspan(std::size_t offset,
    ...
    MICROFMT_ASSERT(actual_count <= rem, "subspan count exceeds remaining span size");
  ...
  unsafe_subspan(std::size_t offset,
    ...
    const std::size_t rem = m_size - offset;
    const std::size_t actual_count = count == static_cast<std::size_t>(-1) ? rem : count;
    MICROFMT_DEBUG_ASSERT(actual_count <= rem, "subspan count exceeds remaining span size");
    return span<T>(pointer_at(offset), actual_count);
  }
