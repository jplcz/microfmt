...
| [Micro VM bytecode scripting](inspector/micro-vm.md) | A zero-allocation stack-machine interpreter, bytecode generator, and a compiled C-like scripting language for register/memory expressions, fault-safe target access, and formatted output | `micro_vm.hpp`, `vm_code_gen.hpp`, `vm_lexer.hpp`, `vm_compiler.hpp`, `vm_label_allocator.hpp`, `variable_context.hpp` |
