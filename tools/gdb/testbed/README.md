...
microfmt's GDB pretty printers (`tools/gdb/microfmt_printers.py` /
`include/microfmt/gdb_printers.hpp`). It is **not** part of the main CMake
...
- `testbed.cpp` -- one `main()` with a small, recognizable instance of every
  microfmt type that has a printer, plus a `// GDB_CHECK: <name> => <substring>`
...
- `testbed_embedded.cpp` -- the exact same program, but `#include
  <microfmt/gdb_printers.hpp>` first, so the printers are embedded via
...
`PATH`. microfmt depends on reloco's headers; if you don't have a sibling
`../reloco` checkout next to this repository, set
`MICROFMT_RELOCO_INCLUDE_DIR` to a reloco checkout's `include/` directory
(mirrors `JPLCZ_MICROFMT_RELOCO_SOURCE_DIR`'s convention for the main CMake
build):

```
MICROFMT_RELOCO_INCLUDE_DIR=/path/to/reloco/include tools/gdb/testbed/run.sh
```

Add `--keep-build` to keep `tools/gdb/testbed/build/` (binaries, generated
`<binary>-gdb.py`, and one transcript per mode) around after a failure for
manual inspection; without it, `build/` is removed on exit (it's already
...
1. **`source-script`** -- `testbed_plain`, printers loaded with an explicit
   `source tools/gdb/microfmt_printers.py`.
...
3. **`embedded-header`** -- `testbed_embedded`, printers auto-loaded purely
   ...
   `include/microfmt/gdb_printers.hpp`; no sidecar file, no `source`
   command.
...
1. In `tools/gdb/microfmt_printers.py`, write or update the printer class
   and its `pp.add_printer(...)` registration.
...
3. In `testbed.cpp`, declare one local variable of the new type, initialized
   ...
   your printer's `to_string()` will render distinctly so a broken printer
   produces an obviously wrong (not just subtly wrong) string.
