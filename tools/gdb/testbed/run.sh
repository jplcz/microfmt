...
#
# microfmt depends on reloco's headers. Point MICROFMT_RELOCO_INCLUDE_DIR at
# a reloco checkout's include/ directory if it isn't a sibling checkout at
# ../reloco relative to this repository's root (matches
# JPLCZ_MICROFMT_RELOCO_SOURCE_DIR's convention used by the main CMake build).
...
CXX="${CXX:-g++}"
GDB="${GDB:-gdb}"
RELOCO_INCLUDE_DIR="${MICROFMT_RELOCO_INCLUDE_DIR:-${REPO_ROOT}/../reloco/include}"
...
if [[ ! -f "${RELOCO_INCLUDE_DIR}/reloco/array.hpp" ]]; then
  echo "error: reloco headers not found under '${RELOCO_INCLUDE_DIR}'" >&2
  echo "       set MICROFMT_RELOCO_INCLUDE_DIR to a reloco checkout's include/ directory" >&2
  exit 1
fi
...
"${CXX}" -std=c++20 -g -O0 -I "${REPO_ROOT}/include" -I "${RELOCO_INCLUDE_DIR}" \
  "${SCRIPT_DIR}/testbed.cpp" -o "${BUILD_DIR}/testbed_plain"

echo "==> Compiling testbed (with include/microfmt/gdb_printers.hpp embedded)"
"${CXX}" -std=c++20 -g -O0 -I "${REPO_ROOT}/include" -I "${RELOCO_INCLUDE_DIR}" \
...
import microfmt_printers  # noqa: F401
PYEOF
...
run_mode "source-script" "${BUILD_DIR}/testbed_plain" \
  -iex "source ${REPO_ROOT}/tools/gdb/microfmt_printers.py" || OVERALL=1
