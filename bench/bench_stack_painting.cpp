...
NOINLINE void run_on_isolated_stack(stack_arena &arena, void (*func)()) noexcept {
  paint_arena(arena);

#if defined(__x86_64__) || defined(__aarch64__) || defined(__arm__)
  const uintptr_t new_sp = (reinterpret_cast<uintptr_t>(arena.memory + sizeof(arena.memory)) & ~uintptr_t{0xF});
#endif
...
template <typename T> [[gnu::always_inline]] inline void do_not_optimize(T const &val) noexcept {
#if defined(__GNUC__) || defined(__clang__)
...
int main() {
  ...
  microfmt::println(MICROFMT_STRING("   microfmt Runtime Stack High-Water Mark Benchmark (1-Byte Res)   "));
  microfmt::println(MICROFMT_STRING("==================================================================="));

  bench::stack::run_on_isolated_stack(bench::stack::g_arena, probe_baseline);
  size_t baseline_stack = bench::stack::measure_high_water_mark(bench::stack::g_arena);

  microfmt::println(MICROFMT_STRING("Baseline Call Overhead: {} Bytes"), baseline_stack);
  microfmt::println(MICROFMT_STRING("-------------------------------------------------------------------"));
  microfmt::println(MICROFMT_STRING("{:<45} | {:>10} | {:>12}"), "Benchmark Probe", "Peak Stack", "Net Overhead");
  microfmt::println(MICROFMT_STRING("-------------------------------------------------------------------"));

  bench_case cases[] = {
      {"microfmt runtime vformat_to (10 args)", probe_microfmt_runtime_10_args},
      {"microfmt compiled MICROFMT_STRING (10 args)", probe_microfmt_compiled_10_args},
  };

  for (const auto &c : cases) {
    bench::stack::run_on_isolated_stack(bench::stack::g_arena, c.fn);
    size_t total_stack = bench::stack::measure_high_water_mark(bench::stack::g_arena);
    size_t net_overhead = (total_stack >= baseline_stack) ? (total_stack - baseline_stack) : 0;

    microfmt::println(MICROFMT_STRING("{:<45} | {:>8} B | {:>10} B"), c.name, total_stack, net_overhead);
  }
