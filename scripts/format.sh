...
#
# The clang-format binary can be overridden via MICROFMT_CLANG_FORMAT
...
clang_format="${MICROFMT_CLANG_FORMAT:-clang-format}"

if ! command -v "${clang_format}" >/dev/null 2>&1; then
  echo "error: '${clang_format}' not found on PATH (set MICROFMT_CLANG_FORMAT to override)" >&2
  exit 1
fi
...
mapfile -d '' files < <(find "${dirs[@]}" \( -name 'build*' -o -name 'cmake-build-*' \) -prune -o -type f \( \
