...
TEST(AddressSpaceRef, ReturnsTypedReadErrorsAndValues) {
  ...
  auto invalid_buffer = local_space().read_bytes(address_of(value), nullptr, sizeof(value));
...
TEST(AddressSpaceRef, ReturnsStringChunkMetadataAndErrors) {
  ...
  auto empty_buffer = local_space().read_string_chunk(address_of(source), microfmt::span<char>{});
  ASSERT_FALSE(empty_buffer);
  EXPECT_EQ(empty_buffer.error(), microfmt::address_space_error::empty_buffer);
}
...
TEST(RemoteRef, ReturnsPreciseLoadErrors) {
  ...
  microfmt::remote_ref<object> too_small(address_of(source), local_space(), small_scratch);
  auto small_result = too_small.load();
  ASSERT_FALSE(small_result);
  EXPECT_EQ(small_result.error(), microfmt::remote_load_error::scratch_too_small);

  alignas(object) std::byte misaligned_storage[sizeof(object) + 1]{};
  microfmt::remote_ref<object> misaligned(address_of(source), local_space(),
  ...
  EXPECT_EQ(misaligned_result.error(), microfmt::remote_load_error::scratch_misaligned);
  ...
  EXPECT_EQ(invalid_space_result.error(), microfmt::remote_load_error::invalid_address_space);
}
...
TEST(AddressTranslatorRef, HandlesEmptyAndStatelessTranslators) {
  ...
  microfmt::translation_attributes attributes{.physical_address = 0x55, .readable = true};
  ...
  auto made = microfmt::address_translator_ref::make<stateless_translator_tag>();
  attributes = {};
...
TEST(MemoryClassifierRef, ChecksHalfOpenRegionBounds) {
  constexpr microfmt::memory_region_info region{
      .start_address = 0x1000, .end_address = 0x2000, .type = microfmt::memory_region_type::kernel_data};
...
TEST(MemoryClassifierRef, HandlesEmptyAndStatelessClassifiers) {
  ...
  microfmt::memory_region_info info{
      .start_address = 1, .end_address = 2, .type = microfmt::memory_region_type::guard_page};
  ...
  auto made = microfmt::memory_classifier_ref::make<stateless_classifier_tag>();
...
TEST(MemoryClassifierRef, ForwardsStateAndClassifiesConfiguredRegions) {
  constexpr microfmt::memory_region_info regions[]{{.start_address = 0x1000,
                                                    .end_address = 0x2000,
...
TEST(MemoryScanner, ScansRawAddressesAndSuppressesUnsafeRegions) {
  ...
  scanner_space_context space_context{.virtual_base = data_address, .data = data.data(), .size = data.size()};
  ...
  const microfmt::memory_region_info regions[]{{.start_address = data_address,
                                               ...
                                               {.start_address = code_address,
                                                ...
                                                .writable = false,
                                                .executable = true}};
  classifier_context classifier_state{regions, sizeof(regions) / sizeof(regions[0])};
  microfmt::memory_classifier_ref classifier(stateful_classifier_tag{}, classifier_state);
  ...
  microfmt::memory_scanner::scan_and_dump<microfmt::x86_64_abi_traits>(
      space, classifier, microfmt::register_context_ref{}, addresses, *scanner_context, output.as_sink());
...
TEST(MemoryScanner, ScansOrderedAddressRegistersAndAbstractSources) {
  ...
  const auto read_register = [](const void *opaque_state, microfmt::address_space_ref, uint32_t index, void *out_value,
  ...
  microfmt::memory_scanner::scan_and_dump<microfmt::aarch64_abi_traits>({}, {}, registers, {}, *scanner_context,
                                                                        register_output.as_sink());
  ...
  EXPECT_NE(source_output.view().find("addr=0x0000000000004444"), microfmt::string_view::npos);
  EXPECT_NE(source_output.view().find("addr=0x0000000000005555"), microfmt::string_view::npos);

  microfmt::address_source_ref empty_source;
  microfmt::buffer_sink<32> empty_output;
  microfmt::memory_scanner::scan_and_dump({}, {}, empty_source, *scanner_context, empty_output.as_sink());
...
bool read_fake_register(const void *opaque_state, microfmt::address_space_ref space, uint32_t dwarf_reg_index,
                        void *out_value, size_t value_size) noexcept {
  const auto &state = *static_cast<const fake_register_state *>(opaque_state);
  ++state.read_calls;
  state.last_read_index = dwarf_reg_index;
  state.last_read_size = value_size;

  int probe = 0;
  state.read_space_forwarded = space.read(state.probe_address, probe) && probe == state.expected_probe;
...
bool write_fake_register(void *opaque_state, microfmt::address_space_ref space, uint32_t dwarf_reg_index,
                         const void *in_value, size_t value_size) noexcept {
  ...
  int probe = 0;
  state.write_space_forwarded = space.read(state.probe_address, probe) && probe == state.expected_probe;
...
bool read_sparse_register(const void *opaque_state, microfmt::address_space_ref, uint32_t dwarf_reg_index,
                          void *out_value, size_t value_size) noexcept {
  const auto &state = *static_cast<const sparse_register_state *>(opaque_state);
...
bool read_register_range(const void *opaque_state, microfmt::address_space_ref, uint32_t dwarf_reg_index,
                         void *out_value, size_t value_size) noexcept {
  const auto &state = *static_cast<const register_range_state *>(opaque_state);
...
constexpr bool address_candidates_are_unique_gprs(const CandidateArray &candidates,
                                                  const RegisterArray &registers) noexcept {
...
TEST(DwarfRegisterTraits, DefinesAllArchitectureRegisterCatalogs) {
  ...
  static_assert(address_candidates_are_unique_gprs(x86_64_traits::address_registers(), x86_64_traits::gpr_registers));
  static_assert(address_candidates_are_unique_gprs(riscv_traits::address_registers(), riscv_traits::gpr_registers));
...
TEST(RegisterContextRef, DispatchesTypedAndRawReadsAndWrites) {
  ...
  const std::array<std::byte, 2> raw_replacement{std::byte{0x34}, std::byte{0x12}};
  EXPECT_TRUE(context.write_raw(20, raw_replacement.data(), raw_replacement.size()));
...
TEST(InspectorContainer, InvokesTypeErasedContextAndReportsFailures) {
  ...
  auto context = microfmt::make_container_context(7, [&invoked](int &state, const microfmt::container_options &opts,
                                                                microfmt::address_space_ref, microfmt::span<std::byte>,
                                                                const microfmt::sink &out) noexcept {
    invoked = true;
    microfmt::format_to(out, "{}{}{}{}", opts.open_bracket, state, opts.entry_separator, opts.close_bracket);
    return true;
  });
  int container = 0;
...
TEST(InspectorSmartPointers, FormatsViewsAndPointerLayoutWrappers) {
  ...
  microfmt::remote_unique_ptr_view<int> unique(address_of(unique_storage), space, scratch);
  microfmt::remote_shared_ptr_view<int> shared_view(
      address_of(shared), space, scratch, offsetof(shared_storage, pointer), offsetof(shared_storage, control_block),
      offsetof(control_block, use_count), offsetof(control_block, weak_count));
  microfmt::remote_intrusive_ptr_view<int> intrusive(address_of(intrusive_storage), space, scratch, 0, 0);
  EXPECT_EQ(microfmt::format<32>("{}", unique).view(), "42");
  EXPECT_EQ(microfmt::format<64>("{}", shared_view).view(), "shared_ptr(42, use_count=3, weak_count=1)");
  EXPECT_EQ(microfmt::format<64>("{}", intrusive).view(), "intrusive_ptr(42, ref_count=42)");

  uintptr_t null_storage = 0;
  EXPECT_EQ(microfmt::format<32>("{}", microfmt::remote_unique_ptr_view<int>(address_of(null_storage), space, scratch))
                .view(),
            "nullptr");
  EXPECT_EQ(microfmt::format<32>("{}", microfmt::remote_shared_ptr_view<int>(address_of(null_storage), space, scratch,
                                                                             0, sizeof(uintptr_t), 0, sizeof(int32_t)))
                .view(),
            "shared_ptr(nullptr)");
  EXPECT_EQ(microfmt::format<32>(
                "{}", microfmt::remote_intrusive_ptr_view<int>(address_of(null_storage), space, scratch, 0, 0))
                .view(),
  ...
  EXPECT_EQ(microfmt::format<32>("{}", intrusive_wrapper).view(),
            microfmt::format<32>("0x{:x}", address_of(value)).view());
  EXPECT_EQ(microfmt::format<32>("{}", microfmt::remote_unique_ptr<int>{}).view(), "nullptr");
  EXPECT_EQ(microfmt::format<32>("{}", microfmt::remote_shared_ptr<int>{}).view(), "shared_ptr(nullptr)");
  EXPECT_EQ(microfmt::format<32>("{}", microfmt::remote_intrusive_ptr<int>{}).view(), "intrusive_ptr(nullptr)");
}
