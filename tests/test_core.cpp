...
struct can_index_temporary_span : std::false_type {};

template <typename Span>
struct can_index_temporary_span<
    Span, std::void_t<decltype(std::declval<Span &&>()[0])>> : std::true_type {};

template <typename Span, typename = void>
struct can_borrow_temporary_span_data : std::false_type {};

template <typename Span>
struct can_borrow_temporary_span_data<
    Span, std::void_t<decltype(std::declval<Span &&>().data())>>
    : std::true_type {};

template <typename Span, typename = void>
struct can_iterate_temporary_span : std::false_type {};

template <typename Span>
struct can_iterate_temporary_span<
    Span, std::void_t<decltype(std::declval<Span &&>().begin())>>
    : std::true_type {};

template <typename Span, typename = void>
struct can_borrow_temporary_span_unsafe_front : std::false_type {};

template <typename Span>
struct can_borrow_temporary_span_unsafe_front<
    Span, std::void_t<decltype(std::declval<Span &&>().unsafe_front())>>
    : std::true_type {};

template <typename Span, typename = void>
struct can_borrow_temporary_span_unsafe_back : std::false_type {};

template <typename Span>
struct can_borrow_temporary_span_unsafe_back<
    Span, std::void_t<decltype(std::declval<Span &&>().unsafe_back())>>
    : std::true_type {};
...
static_assert(
    !can_borrow_temporary_span_unsafe_front<microfmt::span<int>>::value);
static_assert(
    !can_borrow_temporary_span_unsafe_back<microfmt::span<int>>::value);
...
TEST(CoreSpan, ProvidesCheckedAndFallibleAccess) {
  ...
  EXPECT_FALSE(view.try_at(view.size()).has_value());
  EXPECT_EQ(view.try_at(view.size()).error(),
            microfmt::span_error::out_of_bounds);
...
TEST(CoreSpan, StdSpanInteroperability) {
  ...
  static_assert(
      !std::is_convertible_v<microfmt::span<char> &&, std::span<char>>);
}
#endif
