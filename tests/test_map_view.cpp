...
TEST(MapViewTest, FormatsStandardMapWithDefaultExtractors) {
  ...
  const std::map<int, const char *> states{{1, "boot"}, {3, "ready"}, {7, "fault"}};
...
TEST(MapViewTest, SupportsDelimiterAndForwardedElementSpecifiers) {
  ...
  microfmt::format_to(output.as_sink(), "{:b04X}", microfmt::map_view(registers));
  EXPECT_EQ(output.view(), "[0001: 00A1, 000F: BEEF]");

  output.reset();
  microfmt::format_to(output.as_sink(), "{:n02x}", microfmt::map_view(registers));
