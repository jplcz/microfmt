...
TEST(CborWriterTest, EncodesNestedMapAndArrayRoundTrip) {
  ...
  microfmt::buffer_sink<512> output;
  {
    ...
    document.kv("device", "sensor-7").kv("enabled", true).kv("identifier", microfmt::span(identifier));
  ...
  EXPECT_EQ(document["identifier"].get_binary(), nlohmann::json::binary_t({0xDE, 0xAD, 0xBE, 0xEF}));
...
TEST(CborWriterTest, EncodesIntegerBoundariesAndNullCString) {
  ...
  microfmt::buffer_sink<256> output;
  {
    ...
    values.val(std::numeric_limits<int64_t>::min()).val(std::numeric_limits<uint64_t>::max()).val(null_text).val("");
  }
