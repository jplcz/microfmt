...
TEST(PointerTest, FormatsNativeAndCompatibilityWidths) {
  ...
  microfmt::format_to(output.as_sink(), "{}",
                      microfmt::raw_ptr(high_address,
                                        microfmt::ptr_width_mode::compat32));
  EXPECT_EQ(output.view(), "0x9abcdef0");

  output.reset();
  microfmt::format_to(output.as_sink(), "{}",
                      microfmt::raw_ptr(high_address,
                                        microfmt::ptr_width_mode::compat64));
  EXPECT_EQ(output.view(), "0x123456789abcdef0");

  output.reset();
  microfmt::format_to(output.as_sink(), "{}",
                      microfmt::raw_ptr32(high_address));
  EXPECT_EQ(output.view(), "0x9abcdef0");
}
...
TEST(PointerTest, SupportsPrefixCaseWidthAndModeSpecifiers) {
  ...
  microfmt::format_to(output.as_sink(), "{:P}",
                      microfmt::raw_ptr(uintptr_t{0xAB}));
  EXPECT_EQ(output.view(), sizeof(uintptr_t) == 8 ? "0X00000000000000AB"
                                                   : "0X000000AB");

  output.reset();
  microfmt::format_to(output.as_sink(), "{:x}",
                      microfmt::raw_ptr(uintptr_t{0xAB}));
  EXPECT_EQ(output.view(), sizeof(uintptr_t) == 8 ? "00000000000000ab"
                                                   : "000000ab");

  output.reset();
  microfmt::format_to(output.as_sink(), "{:08X}",
                      microfmt::raw_ptr(uintptr_t{0xAB}));
  EXPECT_EQ(output.view(), "000000AB");

  output.reset();
  microfmt::format_to(output.as_sink(), "{:32P}",
                      microfmt::raw_ptr(high_address));
  EXPECT_EQ(output.view(), "0X9ABCDEF0");

  output.reset();
  microfmt::format_to(output.as_sink(), "{:64x}",
                      microfmt::raw_ptr(uintptr_t{0x1234}));
...
TEST(PointerTest, FormatsTypedPointersAndAllNullRepresentations) {
  ...
  output.reset();
  microfmt::format_to(output.as_sink(), "{}",
                      microfmt::raw_ptr(static_cast<int *>(nullptr)));
  ...
  output.reset();
  microfmt::format_to(output.as_sink(), "{:Pz}",
                      microfmt::raw_ptr(uintptr_t{0}));
  EXPECT_EQ(output.view(), "0x0");
}
...
TEST(PointerTest, FormatsPointerRangesFromBeginEndAndCount) {
  ...
  microfmt::format_to(output.as_sink(),
                      "{}", microfmt::raw_range(values.data(),
                                                  values.data() + values.size()));
  EXPECT_EQ(output.view(), "[7, 42, -1]");

  output.reset();
  microfmt::format_to(output.as_sink(), "{}",
                      microfmt::raw_range(values.data() + 1, size_t{2}));
...
TEST(PointerTest, FormatsEmptyAndNullPointerRanges) {
  ...
  output.reset();
  const auto null_range = microfmt::raw_range(static_cast<const int *>(nullptr),
                                               size_t{8});
