...
struct can_get_temporary_array_element<
    Array, std::void_t<decltype(microfmt::get<0>(
...
struct can_deduce_mixed_array<
    T, U,
    std::void_t<decltype(
        microfmt::array{std::declval<T>(), std::declval<U>()})>>
    : std::true_type {};

static_assert(!can_index_temporary_array<microfmt::array<int, 2>>::value);
static_assert(!can_borrow_temporary_array_data<
              microfmt::array<int, 2>>::value);
static_assert(!can_iterate_temporary_array<microfmt::array<int, 2>>::value);
static_assert(
    !can_get_temporary_array_element<microfmt::array<int, 2>>::value);
static_assert(!can_deduce_mixed_array<int, short>::value);
static_assert(std::tuple_size<microfmt::array<int, 3>>::value == 3);

} // namespace

TEST(ArrayTest, SupportsAggregateAccessAndStructuredBindings) {
  microfmt::array values{1, 2, 3};
...
TEST(ArrayTest, ProvidesSpanSubviewsAndMapping) {
  microfmt::array<int, 4> values{1, 2, 3, 4};
...
TEST(ArrayTest, SupportsFillSwapComparisonAndZeroSize) {
  microfmt::array<int, 3> lhs{};
  microfmt::array<int, 3> rhs{};
  lhs.fill(4);
  rhs.fill(9);
